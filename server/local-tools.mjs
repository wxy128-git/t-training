...
function normalizeTool(item) {
    const tool = {
        ...
        category: text(item.category || 'more', 80), taskCategory: text(item.taskCategory, 30), tags: text(item.tags, 120), fit: text(item.fit, 240), reviewedAt: text(item.reviewedAt, 10), order: Number(item.order)
    };
