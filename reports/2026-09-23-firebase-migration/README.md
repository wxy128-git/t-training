...
当前处于“隔离候选验证完成、生产尚未切换”阶段。线上 `t-training-api` 仍使用 Firebase；本地认证和数据读写已在腾讯服务器 `127.0.0.1:3003` 的隔离候选进程中复测，测试结束后候选进程已停止。
...
- 本地会话签名密钥已随机生成并保存在权限为 `600` 的迁移环境文件中；生成前备份为 `/home/ubuntu/.config/t-training-migration.env.pre-auth-secret-20260923`，生产开关仍未启用。
...
- 腾讯云已验证发信域名 `notify.teachailab.com` 并创建 `no-reply@notify.teachailab.com`。由于个人实名认证账号不支持 SMTP，候选改用腾讯云 SES API。邮箱验证模板 `61815` 和密码重置模板 `61816` 均已审核通过。专用编程子用户 `t-training-ses` 只关联 `ses:SendEmail` 权限，凭证和两个模板 ID 已写入服务器权限为 `600` 的私密环境文件；本机下载副本已删除。本地邮件开关仍未启用，当前生产流程没有变化。
...
- 本轮候选发布包保留在 `/home/ubuntu/t-training/staging/20260923-ses-candidate-0eadfdd`，候选配置权限为 `600`；测试进程未写入 PM2 开机清单，并已在测试后停止。
- 候选智能体使用无效令牌返回 401，未发起模型请求。
- 所有测试账号、留言、订阅、会话、邮箱令牌和远端临时脚本均已清理；复测后的数据库数量与最新快照一致。

## 正式切换前仍需完成

1. 在隔离候选服务中完成腾讯云 SES 真实邮件投递测试。
