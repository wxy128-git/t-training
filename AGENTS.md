...
| `tools.html` | Curated directory of **external** third-party AI products (cards link out). User-facing name **「AI资源精选」** (renamed 2026-06-16; nav key stays `tools`). Renders `DEFAULT_TOOLS` sync first, then Firestore (`DB.getTools`). Default "全部" = **grouped by category** (`#tools-grouped`); search/specific category → flat `#tools-grid`; pills from `buildFilterBar()`. **「精选指南」改版 (2026-06-26)** — all CSS in an **inline `<style>` in tools.html** (not style.css): ① **editorial header**: mono kicker「AI · TOOLBOX」+ 朱红「甄选」seal `.tt-seal` + intro. ② **Task-oriented categories (rethought)**: `plan` 备课·出题·检索 / `make` 课件·演示·动画 / `media` 图像·音视频·数字人 / `platform` 教研·AI 平台 (+ `more` 更多工具 fallback). **Category + per-tool tags come from a LOCAL `TOOL_META` map keyed by tool NAME inside tools.html (NOT Firestore)** — `applyMeta()` overrides each tool's `.category` to this scheme + attaches `._tags`. ⚠️ **维护**：在 admin 新增的工具若未登记进 `TOOL_META`，会落进「更多工具」且无标签——需在 tools.html 的 `TOOL_META` 加一行。③ **Card tags**: 免费 / 部分免费 / 付费 / 中文 / 海外 (`.tt-tag` 变体；标签值是先填的估计，需人工核对). ④ **Grid override**: `#tools-grid, #tools-grouped .cards-grid` use `repeat(auto-fit, minmax(208px,264px))` (NOT the shared `.cards-grid-4`) so 1-tool categories aren't surrounded by empty stretched columns. ⑤ 顺带清理了搜索框/卡片残留冷色。(曾加「力荐」featured，2026-06-26 按用户要求移除。) |
...
**退出提速 + 记住我 / 登录持久化 (2026-06-28)**：①`Auth.logout()` 也改**原地**（旧版 `location.reload`，慢）：先 `forgetProxyAuthSession()`+`rememberLastAuthUser(null)`+清 `_currentUser`，立刻 `refreshAuthUI()` 显示未登录，派发 `authRefresh`（备课本→回登录门 / 后台→reload），**最后**才 `await auth.signOut()` 后台清 Firebase 令牌——用户瞬间看到已退出，不再等整页 reload。②登录加「记住我」勾选（`#li-remember`，**默认不勾**，旁注"公用电脑勿勾"）：`Auth.login(identifier, pwd, remember)` 在 signIn **之前** `await auth.setPersistence(remember ? LOCAL : SESSION)`——**勾=LOCAL**（关浏览器仍登录），**不勾=SESSION**（关浏览器即退出，公用机更安全）；常量是 `firebase.auth.Auth.Persistence.LOCAL/.SESSION`。注册固定 `LOCAL`（自家账号长期保持）。⚠️ 安全说明：localStorage 只存「资料快照」做乐观渲染、非令牌，篡改它只能伪造本机 UI，数据由 Firestore 安全规则在服务端兜底，不会泄露。`js/auth.js?v=20260628-remember`（全站同步）。
