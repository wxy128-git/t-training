...
- 腾讯云已验证发信域名 `notify.teachailab.com` 并创建 `no-reply@notify.teachailab.com`。由于个人实名认证账号不支持 SMTP，候选改用腾讯云 SES API。邮箱验证模板 `61815` 已审核通过，密码重置模板 `61816` 正在审核。专用编程子用户 `t-training-ses` 只关联 `ses:SendEmail` 权限，凭证和两个模板 ID 已写入服务器权限为 `600` 的私密环境文件；本机下载副本已删除。本地邮件开关仍未启用，当前生产流程没有变化。
...
1. 等待密码重置模板 `61816` 审核通过，然后在隔离候选服务中完成真实邮件投递测试。
