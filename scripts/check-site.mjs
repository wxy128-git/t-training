...
for (const file of appPages) {
    ...
    const styleVersion = file === 'resources.html' ? '20260924-unified-logo-tiles' : '20260924-local-email-direct';
...
for (const module of ['agent-stream', 'task-search']) {
    if (!agentsHtml.includes(`js/${module}.js?v=20260924-local-email-direct`) || !text('sw.js').includes(`/js/${module}.js`)) fail('agents.html', `${module} 的加载或离线缓存缺失`);
}
...
const emailGateSource = text('js/email-gate.js');
if (/\/api\/email-action|在本站粘贴处理|连接 VPN/.test(authSource + emailGateSource)) {
    fail('js/auth.js / js/email-gate.js', '用户界面仍显示旧 Firebase 邮件链接粘贴入口');
}
