...
- Nine Cloudflare Pages Functions under `functions/api/`:
  ...
  - `email-action.js` — 本站邮箱操作页。支持直接接收 Firebase 邮箱操作参数，也支持老师把打不开的 Firebase 验证、换绑、恢复或密码重置邮件链接复制到 `https://ai.teachailab.com/api/email-action`，由腾讯云 API 代为连接 Firebase 完成一次性操作。页面只接受本项目 Firebase 域名和本站域名，禁用缓存与来源发送，并在读取参数后立即从地址栏移除 `oobCode`；Nginx 对该路径关闭访问日志，避免一次性代码落盘。Firebase 项目当前拒绝修改默认模板的 `callbackUri`（`EMAIL_TEMPLATE_UPDATE_NOT_ALLOWED`），因此不能把“邮件按钮直接打开本站”写成已实现行为。
...
- 后续仍有：共享 SDK 延迟加载、学习路径独立成果任务、完整无障碍认证，以及智谱模型的教学质量复测。
...
- **2026-09-25（后台工具 / 课件素材空列表修复与工具 Logo 流程补齐，已上线）**：
  ...
  - 发布后发现生产服务器的 `sites-available` 与 `sites-enabled` 是两个独立普通文件：首次切回只更新了前者，删除候选进程后生效配置仍请求 3002，导致登录和部分内容接口短时返回 502。已立即把两个文件统一到正式端口 3001 并 reload；无效登录输入恢复为预期 400、工具接口恢复 21 项，完整公网检查再次通过 108 项，账户和业务数据未受影响。部署文档已增加“核对生效配置、完成公网接口验证后再删除候选”的硬性步骤，防止复发。
...
- **2026-09-25（工具 / 课件素材目录与后台 Logo 管理，已上线）**：
  ...
  - 新增 `/api/resource-logo`：仅管理员可自动抓取或上传，拒绝内网地址、特殊端口、过大文件和未识别格式；图片按内容哈希保存到 `/home/ubuntu/t-training/shared/resource-logos`，通过本站长缓存地址读取。未修改现有工具、资源、用户或课程数据。
  - 实现提交 `4f06f23`；发布目录 `/home/ubuntu/t-training/releases/20260925-resource-logo-4f06f23`，回滚备份 `/home/ubuntu/t-training/backups/20260925-pre-resource-logo-4f06f23`。白名单发布包包含 152 个静态文件、24 个 API 文件；共享 Logo 目录权限为 `750`，独立于 release 与正式目录切换。
  - 本地全量回归通过，其中资源 Logo 31 项、腾讯云适配 16 项；3002 候选通过健康、公开内容、认证输入、未授权写入与新路由检查，正式 3001 切回后公网生产检查通过 108 项。教师站和同机教育媒体课程站均为 HTTP/2 200，旧 Cloudflare 地址保留路径和参数 302；`t-training-api` 与 `edu-media` online，`pm2-ubuntu` enabled / active，PM2 启动清单已保存。候选进程、交换目录、本地打包目录和临时检查文件已清理；验证过程未发送邮件、未创建账号、未调用模型。
...
- **2026-09-22（智能体教学质量约束，已上线）**：
  ...
  - 实现提交 `2423bd9`、复测提交 `5e0543d`、发布提交 `62acda1`；`agents-data.js` 缓存版本为 `20260922-teaching-quality`，Service Worker 为 `20260922-v23`。本地全量检查通过：11 个公开页 / 13 个应用页、13 个课程场景、67 项函数、65 项邮箱、19 项生成可靠性、8 项腾讯云适配断言。
  - 发布目录 `/home/ubuntu/t-training/releases/20260922-teaching-quality-62acda1`；回滚备份 `/home/ubuntu/t-training/backups/20260922-pre-teaching-quality-62acda1`。107 个静态文件、12 个 API 文件的 SHA-256 与发布清单一致；本次未修改 Firestore 数据或规则。
  - 3002 候选 API 和正式 3001 API 各通过 9 项健康、公开内容、认证、智能体、备课本、统计及 RSS 守卫检查；公网生产 98 项断言通过，正式智能体脚本和 Service Worker 版本可见。`t-training-api` 与 `edu-media` 均 online，`pm2-ubuntu` enabled / active，教师站与同机教育媒体站均为 HTTP/2 200，旧 Cloudflare 地址保留路径和参数 302，正式 API 错误日志 0 字节。
  - 部署过程未发送邮件、未新建账号、未调用模型。Nginx 已回到 3001，候选进程、交换目录、候选脚本及本地打包目录已清理，保留正式发布包和回滚备份。完整复测证据见 [教学质量复测报告](reports/2026-09-22-teaching-retest/README.md)。
...
- **2026-09-22（邮箱验证与教师备课体验优化，已上线）**：
  ...
  - Firestore 已发布规则集 `projects/xylaoshi-28f6c/rulesets/0fd0e37e-89eb-44ca-8bcd-73c035135dbd`；旧规则集为 `1f370b84-c6df-4e56-8b24-0af2bae47f0a`。107 个静态文件 / 12 个 API 文件及数据库规则与发布源一致。
  - 候选真实 Firebase 测试 16 项、候选 / 正式 API 各 9 项、生产账号与数据库守卫 12 项及公网 96 项检查通过；公网页面在手机和桌面检查无脚本错误。原 Cloudflare 域名保留路径与参数 302；`pm2-ubuntu` enabled / active，教师站与同机教育媒体课程站正常。
  - 真实验证链接使用仅返回链接、不发信的管理员测试模式，未测试真实收件箱投递和大陆网络下 Firebase 托管邮件页面的可达性。完整实施、验收及清理记录见 [邮箱验证报告](reports/2026-09-22-email-verification/README.md)。
  - 发布验收创建的 3 个临时账号、5 条测试文档已删除并回查；候选进程、临时发布工作区及交换目录已清理，保留发布包与回滚备份。正式 API 错误日志为 0 字节。
...
- **2026-08-24（备课本视觉减重，已上线）**:
  ...
  - 成果打开动效改为轻微上移淡入；移动端隐藏装订轨，顶部索引完整保留。1440×1024 列表 / 卡片 / 成果弹层与 390×844 手机视口均已浏览器验收，装饰元素计数为 0、手机无横向溢出、运行时错误为 0；站点检查同步加入轻量备课本结构断言。

- **2026-08-24（教材版本定位与生成前硬校验，已上线）**:
  - 新增 `js/textbook-catalog.js`，以教育部《2024年义务教育国家课程教学用书目录（根据2022年版课程标准修订）》为版本范围依据，按小学 / 初中 / 高中、学科提供教材版本候选；公开目录无法证明具体课题所在册次时，不声称自动命中，要求教师查看手边教材目录后确认。高中与学校自选版本明确标注“以学校实际用书为准”。
  - 具备学科、年级字段的备课 / 出题类智能体新增“教材校验章”：支持教材同步、版本、册次和非教材主题。跨学科、独立学科起始年级错误、明显超纲组合在登录及模型调用前硬拦截；非教材主题只跳过版本册次定位，不绕过学科和年级检查。教材身份与禁止虚构单元、页码的边界一并写入模型提示。
  - 新增 `scripts/test-textbook-catalog.mjs` 并纳入 `npm run check`；8 个教材校验场景、站点检查、48 项函数回归和 8 项腾讯云适配层断言通过。隔离 Chrome 验证九年级化学正常对齐、小学语文填写二次函数被拦截、未知课题需教师核对、非教材模式字段停用；390×844 无横向溢出，控件高度均为 40px。Service Worker 已升级为 `VERSION=20260824-v17`。
  - 与上方备课本视觉减重合并发布；实现提交：`57d9b1f`（`上线教材校验与轻量备课本`）。腾讯云发布目录 `/home/ubuntu/t-training/releases/20260824-textbook-workbook-57d9b1f`，回滚备份 `/home/ubuntu/t-training/backups/20260824-pre-textbook-workbook-57d9b1f`；Service Worker 为 `VERSION=20260824-v17`。
  - 发布采用 3002 候选 API + Nginx 热切换；候选健康、工具、公开内容、认证 / 备课本 / 智能体输入守卫和 RSS SSRF 守卫通过后切换。服务器 102 个静态文件、10 个 API 文件与发布包校验和零差异，公网生产检查 95 项通过；Cloudflare 旧地址继续保留路径与查询参数 302，`pm2-ubuntu` enabled/active，`edu-media` 与 `t-training-api` 均 online，教师培训站与教育媒体课程站均返回 HTTP/2 200。

- **2026-08-24（智能体目录渐进展示与数字成员个人工作台，已上线）**:
  - `agents.html` 默认页从“19 人连续陈列”改为“4 位推荐成员 + 5 个教研部门入口”：只让推荐成员使用大幅肖像，部门目录以职责说明、成员数量和少量头像预览建立团队索引；点击部门后只展示该部门 2–5 位成员，搜索仍直接返回匹配成员，另保留次要的“查看全部 19 位成员”入口。首屏右侧取消重复的“今日在席”人脸，改为纯文字团队索引；部门筛选、`?cat=` 深链与 19 个成员均保留。
  - 17 个表单型数字成员工作台从“参数面板”重构为“任务简报 → 成员处理 → 教师核验”：顶部持续显示成员肖像、在岗状态和三阶段进度，左侧由成员用第一人称了解任务，右侧明确标注为该成员的交付区；每位成员只前置最关键的 1–2 项，学科、年级、课时等背景压缩为一行，其余既有参数收进“展开全部参数”。所有原字段、草稿保存、必填校验、项目背景复用、模型调用、结果编辑与备课本保存逻辑继续保留，对话型成员保持原流程。
  - 视觉验收按选定稿完成 1440×1024 同状态桌面对照和 390×844 真实手机视口检查，参数展开、任务摘要同步及三阶段切换通过浏览器交互验证。页面文案缓存版本为 `20260824-agent-directory`，Service Worker 为 `VERSION=20260824-v16`。
  - 实现提交：`351e6c3`（`优化数字教研团队与成员工作台`）。腾讯云发布目录 `/home/ubuntu/t-training/releases/20260824-agent-workspace-351e6c3`，回滚备份 `/home/ubuntu/t-training/backups/20260824-pre-agent-workspace-351e6c3`。
  - 发布采用 3002 候选 API + Nginx 热切换：候选健康、工具、公开内容、认证/备课本守卫和 RSS SSRF 守卫均通过后切换；服务器 101 个静态文件、10 个 API 文件与发布清单 SHA-256 零差异。公网生产检查 90 项通过，Cloudflare 旧地址继续保留路径与查询参数 302；`pm2-ubuntu` enabled/active，`edu-media` 与 `t-training-api` 均 online，教师培训站与教育媒体课程站均返回 200。
