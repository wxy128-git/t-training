...
- **2026-07-08（智能体课程适配校验 + 账号切换清理）**:
  ...
  - `js/agents-data.js` 强化所有相关智能体的 system 提示词：生成前必须先判断学科、年级/学段、课题/知识点/材料是否符合大陆中小学课程体系和学生认知基础；明显不匹配时先输出“参数需要调整”和修改建议，不硬生成。`buildUserPrompt()` 也对所有接入 `CURRICULUM_VALIDATION` 的智能体追加同类校验要求作为兜底；对话式「苏格拉底助手」则要求缺学段时先追问，学段不匹配时降到前置经验。
