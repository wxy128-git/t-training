...
- **2026-08-24（教材版本定位与生成前硬校验，本地待上线）**:
  ...
  - 新增 `scripts/test-textbook-catalog.mjs` 并纳入 `npm run check`；8 个教材校验场景、站点检查、48 项函数回归和 8 项腾讯云适配层断言通过。隔离 Chrome 验证九年级化学正常对齐、小学语文填写二次函数被拦截、未知课题需教师核对、非教材模式字段停用；390×844 无横向溢出，控件高度均为 40px。Service Worker 已预备升级为 `VERSION=20260824-v17`。
