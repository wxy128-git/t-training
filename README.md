...
使用 Nginx + PM2/Node + 腾讯 MariaDB；迁移第一阶段保留 Firebase 作为旧账号首次登录校验和新注册回滚桥接。原
...
## 2026-09-24 Firebase 迁移第一阶段（已上线）

生产登录会话和站内数据已切到腾讯 MariaDB，已有账号首次登录会由 Firebase 校验一次原密码并迁移为本地 `scrypt` 哈希；新注册暂时继续同步到 Firebase，待腾讯 SES 真实链接验收后再启用完全本地注册。邮箱验证与密码重置已使用腾讯云 SES，链接直接回到 `ai.teachailab.com`，不需要 VPN。

最终只读快照包含 373 个账号和 11,067 条文档，哈希校验与严格镜像导入均通过。发布提交为 `74a8666`，公网生产检查通过 103 项；两封经授权的真实测试邮件已被腾讯云接受，仍需完成收件箱内的验证与重置链接点击。
