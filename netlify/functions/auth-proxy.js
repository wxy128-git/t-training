...
function userFromAuth(data, profile = {}) {
    ...
    return {
        ...
        isAdmin: profile.isAdmin === true || rawEmail === 'admin@xylaoshi.com',
...
exports.handler = async function(event) {
    ...
    if (!email || !password || password.length < 6) {
        return json(400, { ok: false, msg: '请填写有效账号和至少 6 位密码' });
    }
    ...
    try {
        if (action === 'register') {
            ...
            return json(200, { ok: true, ...authData, user });
        }
        ...
        return json(400, { ok: false, msg: '未知认证操作' });
    } catch(error) {
        return json(400, { ok: false, msg: error.message, code: error.code || 'AUTH_PROXY_ERROR' });
    }
};
