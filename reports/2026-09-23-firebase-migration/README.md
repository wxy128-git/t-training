...
- 腾讯云已验证发信域名 `notify.teachailab.com` 并创建 `no-reply@notify.teachailab.com`。由于个人实名认证账号不支持 SMTP，候选改用腾讯云 SES API；邮箱验证和密码重置两个 HTML 模板已准备好，等待在控制台提交并审核。服务器尚未配置 SES API 凭证，当前测试不向外发送。
...
1. 在腾讯云提交并审核邮箱验证、密码重置两个发信模板，创建仅有 SES 发信权限的 API 身份，把模板 ID 和密钥写入服务器私密环境文件。
